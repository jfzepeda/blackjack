...
function renderHand(cards) { return cards.map(renderCard).join(''); }
...
function handResultBadge(h) {
  ...
  if (h.result === 'bust')      return `<span class="badge bust">BUST -${h.bet}</span>`;
  if (h.result === 'surrender') return `<span class="badge push">RENDIDO -${Math.ceil(h.bet/2)}</span>`;
  if (h.status === 'busted')    return `<span class="badge bust">BUST</span>`;
  if (h.status === 'blackjack') return `<span class="badge bj">BJ!</span>`;
  if (h.status === 'surrendered') return `<span class="badge push">RENDIDO</span>`;
  if (h.status === 'stood' && h.doubled) return `<span class="badge stood">DOBLADO</span>`;
...
function renderSideBets(p) {
  ...
  const totalStr = (p.sideTotal != null) ? ` <span class="sb-total">Σ=${p.sideTotal}</span>` : '';
  return `<div class="side-bets">${cell('under','Menor13',sb.under)}${cell('exact','★13 jackpot',sb.exact)}${cell('over','Mayor13',sb.over)}${totalStr}</div>`;
}
...
function renderPlayer(p, state) {
  ...
  const handsHtml = p.hands.length === 0
    ...
    : p.hands.map((h) => {
        ...
        return `<div class="${hcls.join(' ')}">
          ...
          <div class="hand-meta">
            ...
            <span class="hand-bet">${h.bet ? `${h.bet}🪙${h.doubled ? ' (x2)' : ''}` : ''}</span>
            ${handResultBadge(h)}
          </div>
        </div>`;
      }).join('');
...
function renderControls(state) {
  ...
  if (me.chips < MIN_BET && state.phase === 'betting') {
    controlsEl.innerHTML = `<span class="bet-info danger">Sin fichas suficientes. ${isHost
      ? 'Tú eres el HOST — usa el botón de tu propio panel.'
      : 'Pide al HOST que te dé fichas.'}</span>`;
    if (isHost) {
  ...
  if (state.phase === 'playing' && state.currentTurn === myId) {
    const hand = me.hands[me.hands.findIndex((h) => h.isCurrent)] || me.hands.find((h) => h.status === 'playing');
    if (!hand) {
    ...
    const sameRank   = isFirstAction && hand.cards[0].r && hand.cards[1].r &&
    ...
    controlsEl.innerHTML = `
      ...
      <button class="btn-primary" id="doubleBtn" ${canDouble ? '' : 'disabled'}>Doblar</button>
      <button class="btn-primary" id="splitBtn"  ${canSplit  ? '' : 'disabled'}>Dividir</button>
      <button class="btn-ghost"   id="surrBtn"   ${canSurr   ? '' : 'disabled'}>Rendirse</button>
      <span class="bet-info">Mano ${me.currentHandIdx ?? 0}: <strong>${hand.total}</strong> · Apuesta <strong>${hand.bet}🪙</strong> · Fichas <strong>${me.chips}🪙</strong></span>
    `;
...
function renderBettingControls(me) {
  ...
  const isDirty = pending.main !== placed.main || pending.under !== placed.under ||
                  pending.exact !== placed.exact || pending.over !== placed.over;
  // Available chips after subtracting NEW intended bets
  const totalRequested = (pending.main || placed.main) + (pending.under || placed.under) +
                        (pending.exact || placed.exact) + (pending.over || placed.over);
  const totalBank = me.chips + placed.main + placed.under + placed.exact + placed.over;
  ...
  const slotBtn = (key, title, sub) => {
    ...
    const diffStr = diff > 0 ? ` <span class="diff">+${diff}</span>` : (diff < 0 ? ` <span class="diff neg">${diff}</span>` : '');
  ...
  controlsEl.innerHTML = `
    ...
    <div class="chip-row">
      ...
      <button class="btn-success" id="confirmBet" ${show.main < MIN_BET || remaining < 0 ? 'disabled' : ''}>
        ${isDirty ? 'Confirmar' : 'OK'} ${show.main}+${show.under + show.exact + show.over}
      </button>
    </div>
    <div class="bet-info">Banco: <strong>${remaining}🪙</strong> de ${totalBank} · ${isDirty ? '<em>sin confirmar</em>' : 'apuesta confirmada'}</div>
  `;
  controlsEl.querySelectorAll('.bet-slot').forEach((s) => {
    s.onclick = () => { activeSlot = s.dataset.slot; renderBettingControls(me); };
  });
  controlsEl.querySelectorAll('.chip-btn').forEach((b) => {
    b.onclick = () => {
      ...
      if (pending.main === 0 && pending.under === 0 && pending.exact === 0 && pending.over === 0) {
        pending = { ...placed };
      }
      const tentative = { ...pending };
      tentative[activeSlot] = (tentative[activeSlot] || 0) + amt;
      const total = tentative.main + tentative.under + tentative.exact + tentative.over;
      if (total > totalBank) {
  ...
  $('confirmBet').onclick = () => {
    ...
    if (finalBet.main < MIN_BET) { showToast(`Apuesta mínima ${MIN_BET}.`); return; }
